...
void loop() {
  ...
  media = map(media,20,900,0,100);   //anota nova
  delta = media-posicao;
  ...
  if ((delta<-4) && (posicao>2) && (media<10)){      //se deslocamento negativo e dentro dos limites
    stepper.move(-100);
    posicao--;
  }
  if ((delta>4) && (posicao<100)){      //se deslocamento positivo e dentro dos limites
    stepper.move(100);
    posicao++;
  }
  media=0;
